...
public class UnstableChargeLaser : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        laserTickRateCounter = 0;                                                   // Set Timer
        laserCooldownCounter = abilities.laserCooldown;                             // Set Timer
        damage = abilities.unstableChargeDamage;
        chargeEnergyCost = ps.maxEnergy / 2;
    }
    ...
    {
        if (laserReady && ps.CanConsumeEnergy(chargeEnergyCost))
        {
    ...
    IEnumerator LaserDuration()
    {
        ...
        laserCooldownCounter = abilities.laserCooldown;                             // Reset the laser cooldown timer
    ...
    {
        yield return new WaitForSecondsRealtime(abilities.laserCooldown);           // Wait for the specified amount of time, in this case the laser's cooldown
