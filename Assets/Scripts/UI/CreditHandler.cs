...
public class CreditHandler : MonoBehaviour
{
    ...
    [SerializeField]bool bigCredit;
    [SerializeField]bool smallCredit;
    ...
    void Start()
    {
        ...
        {
            Vector3 lastPosition = new Vector3(center.transform.position.x, 0, 0);
            
            for (int i = 0; i < headers.Count; i++)
            {
                GameObject newObj = newText(headers[i], true);
                Vector3 nextPosition = new Vector3(center.transform.position.x, lastPosition.y - (Screen.height / screenSpaceInDivisions), 0);
                newObj.transform.position = nextPosition;
                lastPosition = nextPosition;
                creditsTexts.Add(newObj);
                for (int j = 0; j < titles[i].Count; j++)
                {
                    nextPosition = new Vector3(center.transform.position.x, lastPosition.y - (Screen.height / screenSpaceInDivisions), 0);
                    GameObject oObj = newText(titles[i][j], false);
                    oObj.transform.position = nextPosition;
                    creditsTexts.Add(oObj);
                    lastPosition = nextPosition;
